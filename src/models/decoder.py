...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, heads, d_ff, dropout, sep_dec=False):
    ...
    def forward(self, inputs, memory_bank, src_pad_mask, tgt_pad_mask,
                previous_input=None, layer_cache=None, step=None, tgt_segs=None):
        """
        Args:
        ...
        mid = self.context_attn(memory_bank, memory_bank, query_norm,
                                      mask=src_pad_mask,
                                      layer_cache=layer_cache,
                                      type="context", tgt_segs=tgt_segs)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, num_layers, d_model, heads, d_ff, dropout, embeddings, sep_dec=False):
        ...
        self.transformer_layers = nn.ModuleList(
            [TransformerDecoderLayer(d_sep_model, heads, d_ff, dropout, sep_dec=sep_dec)
             for _ in range(num_layers)])

        self.layer_norm = nn.LayerNorm(d_model, eps=1e-6)

    def forward(self, tgt, memory_bank, state, memory_lengths=None,
                step=None, cache=None,memory_masks=None, tgt_segs=None):
        """
